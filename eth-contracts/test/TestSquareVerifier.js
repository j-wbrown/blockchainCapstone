...
var zokratesVerifier = artifacts.require('Verifier');
...
contract('testing zokrates verifier', accounts => {
    const account = accounts[0];

    describe('test verification with correct proof', function () {
        beforeEach(async function () { 
            this.contract = await zokratesVerifier.new({from: account});
        })

        it('test verification is successful', async function () {
            var isVerified = await this.contract.verifyTx(proofData.proof.a,proofData.proof.b,
                                                          proofData.proof.c, proofData.inputs,
                                                          {from: account});
            assert.equal(isVerified, true, "verification is not valid - it should have passed");
        })
    });

    describe('testing verification with incorrect proof', function () {
        beforeEach(async function () { 
            this.contract = await zokratesVerifier.new({from: account});
        })

        it('test verification is unsuccessful', async function () {
            var isVerified = await this.contract.verifyTx(proofData.proof.a,proofData.proof.b,
                                                          proofData.proof.a,proofData.inputs,
                                                         {from: account});
            assert.equal(isVerified, false, "verification is valid - it should have failed");
        })
